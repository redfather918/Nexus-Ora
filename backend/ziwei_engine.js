...
// ─────────────── 五虎遁（安宫干） ───────────────
// 寅宫天干 = 年干对应起始（口诀：甲己丙作首、乙庚戊为头、丙辛寻庚起、丁壬壬位流、戊癸甲寅求）
// 值为 GAN 数组下标（寅宫天干）
const WUHU_DUN = {
  '甲': 2, '己': 2,  // 丙
  '乙': 4, '庚': 4,  // 戊
...
// 四化代号（钦天四化以 ABCD 代禄权科忌）
const SI_HUA_CODE = { '化禄':'A', '化权':'B', '化科':'C', '化忌':'D' };

// 四化象义（钦天派核心：禄多缘、权善变、科情份、忌亏欠，无吉凶之分只分态度）
const SI_HUA_MEANING = {
  '化禄': { code:'A', title:'缘起', desc:'生发、多缘、喜悦，代表与生俱来的天赋资源与缘分', color:'#4ADE80' },
  '化权': { code:'B', title:'变化', desc:'争权、善变、竞争，代表能力、掌控与自我突破', color:'#60A5FA' },
  '化科': { code:'C', title:'情份', desc:'念旧、情深、解厄，代表贵人、名声与稳定平顺', color:'#C084FC' },
  '化忌': { code:'D', title:'亏欠', desc:'执迷、阻滞、亏欠，代表人生课题、执着与需修之处', color:'#F87171' },
};

// ─────────────── 钦天四化（北派飞星）核心算法 ───────────────

/**
 * 五虎遁：根据年干求寅宫天干，再顺时针铺满十二宫宫干
 * @param {string} yearGan 年干（甲/乙/...）
 * @returns {Array<string>} 长度 12，下标为地支序(0=子...11=亥) 的宫干
 */
function getPalaceStems(yearGan) {
  const yinGanIdx = WUHU_DUN[yearGan] !== undefined ? WUHU_DUN[yearGan] : 2; // 默认丙
  const stems = new Array(12).fill('');
  for (let z = 0; z < 12; z++) {
    // 寅=地支序2 起头，顺时针（地支序递增）
    const offset = ((z - 2) % 12 + 12) % 12;
    stems[z] = GAN[(yinGanIdx + offset) % 10];
  }
  return stems;
}
...
/**
 * 钦天四化完整计算
 * @param {Array} palaces 已排好星的 12 宫（含 zhiIdx / name / stars）
 * @param {string} yearGan 年干
 * @returns {object} 钦天四化分析对象
 */
function computeQinTian(palaces, yearGan) {
  const palaceStems = getPalaceStems(yearGan);

  // 1) 给每宫标注宫干（写入 palace.gan，供前端展示）
  palaces.forEach(p => { p.gan = palaceStems[p.zhiIdx] || ''; });

  // 星曜 -> 所在宫索引 的映射（每颗星唯一落宫）
  const starToPalace = {};
  palaces.forEach(p => {
    (p.stars || []).forEach(s => {
      if (s.name && starToPalace[s.name] === undefined) starToPalace[s.name] = p.zhiIdx;
    });
  });

  // 2) 来因宫：宫干 == 年干 之宫（钦天派"我为何而来"之根）
  let laiYin = null;
  for (const p of palaces) {
    if (p.gan === yearGan) { laiYin = { name: p.name, zhi: p.zhi, zhiIdx: p.zhiIdx, gan: p.gan }; break; }
  }

  // 3) 生年四化落宫（A禄 B权 C科 D忌 -> 星 -> 宫）
  const shengNian = [];
  const sn = SI_HUA_TABLE[yearGan] || {};
  for (const hua of ['化禄', '化权', '化科', '化忌']) {
    const star = sn[hua];
    const zhiIdx = star ? starToPalace[star] : undefined;
    const palace = zhiIdx !== undefined ? palaces[zhiIdx] : null;
    shengNian.push({
      hua,
      code: SI_HUA_CODE[hua],
      star: star || '—',
      palaceName: palace ? palace.name : '—',
      zhi: palace ? palace.zhi : '—',
      zhiIdx: zhiIdx !== undefined ? zhiIdx : -1,
      meaning: SI_HUA_MEANING[hua],
    });
  }

  // 4) 十二宫宫干四化飞星（每宫 4 组，共 48 条 A/B/C/D 飞化轨迹）
  const flyingStars = [];
  palaces.forEach(from => {
    const g = from.gan;
    const tbl = SI_HUA_TABLE[g];
    if (!tbl) return;
    for (const hua of ['化禄', '化权', '化科', '化忌']) {
      const star = tbl[hua];
      const toIdx = star ? starToPalace[star] : undefined;
      const toPalace = toIdx !== undefined ? palaces[toIdx] : null;
      flyingStars.push({
        fromGong: from.name,
        fromZhi: from.zhi,
        fromGan: g,
        ...
        code: SI_HUA_CODE[hua],
        star: star || '—',
        isSelf: toIdx === from.zhiIdx,           // 飞入本宫 = 离心自化
        toGong: toPalace ? toPalace.name : '—',
        toZhi: toPalace ? toPalace.zhi : '—',
        toZhiIdx: toIdx !== undefined ? toIdx : -1,
      });
    }
  });

  // 5) 自化识别
  // 离心自化：本宫宫干所化之星，恰落在本宫（能量出而不留）
  const ziHua = flyingStars.filter(f => f.isSelf).map(f => ({
    gong: f.fromGong, zhi: f.fromZhi, gan: f.fromGan,
    hua: f.hua, code: f.code, star: f.star,
  }));
  // 向心自化：对宫宫干所化之星，飞入本宫（能量自外挤压）
  const xinHua = [];
  palaces.forEach(p => {
    const oppIdx = (p.zhiIdx + 6) % 12;
    const opp = palaces[oppIdx];
    const tbl = SI_HUA_TABLE[opp.gan];
    if (!tbl) return;
    for (const hua of ['化禄', '化权', '化科', '化忌']) {
      const star = tbl[hua];
      const toIdx = star ? starToPalace[star] : undefined;
      if (toIdx === p.zhiIdx) {
        xinHua.push({
          gong: p.name, zhi: p.zhi, gan: p.gan,
          hua, code: SI_HUA_CODE[hua], star,
          fromGong: opp.name,
        });
      }
    }
  ...
  // 6) 飞星轨迹按"来源宫"分组（便于前端按宫查看 A/B/C/D 飞往何方）
  const flyingByPalace = palaces.map(p => {
    const list = flyingStars.filter(f => f.fromGong === p.name);
    return { gong: p.name, zhi: p.zhi, gan: p.gan, flies: list };
  });
...
function ziwei({ year, month, day, hour, minute = 0, gender }) {
    ...
    // 13. 钦天四化（北派飞星）计算
    const qinTian = computeQinTian(palaces, yearGan);
